...
export MAP_STORE="$map_store"
export PYTHONPATH="packages/board-core:packages/principal-sdk:adapters/filesystem:apps/board-interface/lark-topic-board"

printf 'Lark topic board smoke: verify event routing, topic lifecycle, exit codes\n'

# Step 1: Notification routing
python3 << 'PYEOF'
import os
...
print('OK generated ' + str(len(notifications)) + ' notifications from ' + str(len(events)) + ' events')
